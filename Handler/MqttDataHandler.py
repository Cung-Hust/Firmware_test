        ...
        try:
            ...
            for ver in list_vers:
                if version.parse(ver) > version.parse(current_ver):
                    new_list_vers.append(ver)
            print(new_list_vers)

            for new_ver in new_list_vers:
                link = f"https://github.com/Cung-Hust/Firmware_test/raw/main/{new_ver}.tar.xz"
                link_dl = "wget " + link
                os.system(link_dl)
                os.system(f'tar -xf {new_ver}.tar.xz')

                # move old file to dir RECOVERY
                os.system('mv RDhcPy/ RECOVERY')
                os.system('mv *.ipk RECOVERY')
                os.system(f'rm {new_ver}.tar.xz')

                # move new file to dir root
                os.system(f'mv /root/{new_ver}/* /root/')
                os.system(f'rm -r {new_ver}/')
